...
//Used: https://stackoverflow.com/questions/49608180/about-threads-and-condition-variable-and-synchronise as reference
//Create monitor system
pthread_cond_t cond1 = PTHREAD_COND_INITIALIZER; //condition for empty
pthread_cond_t cond2 = PTHREAD_COND_INITIALIZER; //condition for full
...
long enqueue_buffer_421(char * data) {
  ...
  if(buffer.read){
    //monitor_count = buffer.length;
    
    //sem_wait(&empty_count); //decrement
    //if(monitor_count < SIZE_OF_BUFFER){
    ...
    if(buffer.length != 0){
      ...
      pthread_cond_signal(&cond2); //allow other thread to process provided buffer not empty
    }
...
// Write your code to enqueue data into the buffer
  ...
  if(buffer.read){
    ...
    if(buffer.length < SIZE_OF_BUFFER){
      ...
      pthread_cond_signal(&cond1); //allow other thread to process provided buffer not empty
    }
